...
int SocketAPI::listenReplicaSocket(replica_struct *packet)
{
    //Associates a local address with a socket and catch errors.
    // cout << "SocketAPI>listenSocket> listening on port = " << this->port << endl;
    ...
    int packetSize = sizeof(replica_struct);
    char buffer[1024];
    bzero(buffer, 1024);
...
int SocketAPI::sendReplicaPacket(replica_struct *packet, string destIP, uint16_t destPort)
{
